...
stored is $1-\alpha$. The optimal strategy will only consider selling
when the price is a local maximum. (If $p_{t-1} > p_t$, then it's
clearly better to sell at $t-1$ than to sell at~$t$; if $p_t <
p_{t+1}$ then it's better to arrive at $t+1$ full than to have to
repurchase at~$t+1$.) So consider a local maximum of price, at time
$t_1$, say. 

If there is no later price that is greater than $p_{t_1}$ then it is
clearly optimal to sell at $t_1$. Otherwise, let $t_2$ be the first
time after $t_1$ such that $p_{t_2} > p_{t_1}$. Between $t_1$ and
$t_2$ there must be at least one local minimum of price; and, of these
minima, a smallest. Let $t_3$ be the time at which the price is a
minimum over the entire interval between $t_1$ and~$t_2$.

Now, we consider whether to sell at $t_1$
