...
maximum. (The operator should clearly sell or buy the maximum possible
at each time step. Consider the case when $p_{t+1} > p_t$ and suppose
that the storage is empty at time~$t$. If the optimal state at $t+1$
is to be empty, it is better to buy at $t$ and sell at $t+1$; if the
optimal state at $t+1$ is to be full, it is better to buy at $t$. In
any event, therefore, it is better to buy at~$t$. The same argument
applies \emph{mutatis mutandis} to the case when the storage is full
at $t$, and to the two cases when $p_{t+1} < p_t$.)
